...
def home(request):
    # ... resto de tu lógica (banners, categorías, etc.)
    ...
    try:
        ...
        if Review is not None:
            approved_reviews = (
                ...
                .only('name', 'rating', 'comment', 'created_at', 'is_approved')  # <- incluye created_at
                .order_by('-created_at')  # <- CAMBIO CLAVE: antes era 'created'
            )[:12]
    except Exception:
        approved_reviews = []
